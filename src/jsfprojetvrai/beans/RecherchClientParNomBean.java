...
@ManagedBean(name="recherchClientParNomBean")
@SessionScoped


public class RecherchClientParNomBean {
	...
	private String userName;             
	 ...
	 Connection connection;
	   ...
	   // String rs;
		...
		public void dbData(String varNom)
	    {
	        try
	        {
	         ...
	         connection = DriverManager.getConnection("jdbc:mysql://localhost:3306/indigopro","root","jsfp");
	            ...
	            SQL = "SELECT nom FROM indigopro.clients WHERE nom LIKE ('"+varNom+"')";
	            
	            rs = statement.executeQuery(SQL); 
	            
	   
	            rs.next();
	            dbpassword = rs.getString("code");//.toString();          
		...
		public String rechClient()
	    {
	        dbData(varNom);  // c'est ici que nous gérons le contenu de la variable pour vérification
	 
	        if(varNom.equalsIgnoreCase(dbuserName))
	        {
	 
	           
	            	   return "recherhClient";
	            	   }
	            else
	            {
	                return "failure";
